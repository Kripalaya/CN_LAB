...
double evaluate_expression(const char *expr, int *ok) {
    ...
    *ok = 1;
    ...
    if (sscanf(expr, "sin(%lf)", &x) == 1) result = sin(x);
    else if (sscanf(expr, "cos(%lf)", &x) == 1) result = cos(x);
    else if (sscanf(expr, "tan(%lf)", &x) == 1) result = tan(x);
    else if (sscanf(expr, "sqrt(%lf)", &x) == 1) {
        if (x < 0) { *ok = 0; return 0.0; }
        result = sqrt(x);
    }
    else if (sscanf(expr, "inv(%lf)", &x) == 1) {
        if (x == 0) { *ok = 0; return 0.0; }
        result = 1.0 / x;
    }
    else if (sscanf(expr, "%lf+%lf", &x, &y) == 2) result = x + y;
    else if (sscanf(expr, "%lf-%lf", &x, &y) == 2) result = x - y;
    else if (sscanf(expr, "%lf*%lf", &x, &y) == 2) result = x * y;
